...
const Job_Close_Notes = (props) => {
  ...
  const getJobNotes = useCallback(async (job) => {
    const _NOTES_PARAM = {
      "condition": { "organisation_id": ORGANISATION_ID, "job_id": job.id, "note_key": environment.NOTES.JOB_NOTES }
    };
    ...
    if (jobNotes.data[0] !== undefined) {
      ...
      if (jobNotes.data[0] !== undefined) { setJobNotesText(jobNotes.data[0].note) }
  ...
  const setJobStatusClosed = useCallback(async () => {
    ...
    await trackPromise(
      DefaultService.PUT(`/job/f_t/closed/${job.id}`, { field_technician_id: TECH_ID })
    );
    history.push({
          ...
          <Col span={24}>
            <Card gutter={5} key={job.id} title={<Title level={5}>{job.customer.full_name}</Title>} extra={<b>Job ID:{job.id}</b>}>
              <Space span={24} direction='vertical' size='small' style={{ display: "flex" }}>
                <Row>
                  <Col span={24}>
                    <textarea onChange={e => setJobNotesText(e.target.value)} style={{ height: "500px", width: "98%" }} value={jobNotesText}></textarea>
                  </Col>
                </Row>

                <Row>
                  <Col span={12}>
                    <Button type='danger' className='ant-btn-lg' style={{ width: "90%" }} onClick={setJobStatusClosed}>
                      {t("general_btn_save_close_job_card")}
                    </Button>
                  </Col>
                  <Col span={12}>
                    <Button
