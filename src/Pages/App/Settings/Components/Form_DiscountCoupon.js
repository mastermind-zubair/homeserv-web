...
const { Modal, Form, Input, Radio, DatePicker, InputNumber } = require("antd");

const { LoadingPanelForPopup } = require("Layout/LoadingPanels");

const Form_DiscountCoupon = ({
  form,
  showForm,
  recordToEdit,
  handleCancel,
  ...
  return (
    <Modal
      title={t("general_add_edit_dis_coup")}
      visible={showForm}
      width={480}
      // onOk={handleSave}
    ...
    >
      ...
      {recordToEdit && (
        ...
        >
          ...
          <Form.Item
            name="code"
            label={t("general_code")}
            className="two-row-item"
            rules={[{ required: true, message: "Please provide a code" }]}
          >
          ...
          <Form.Item
            name="amount"
            label={t("dashboard_job_search_amount")}
            className="two-row-item"
...
export default Form_DiscountCoupon;
