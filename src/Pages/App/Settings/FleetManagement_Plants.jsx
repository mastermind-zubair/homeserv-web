...
import Form_Fleet_Plant from "./Components/Form_Fleet_Plant";
...
const FleetManagement_Plants = (props) => {
  const ENTITY = "Plant Machine";
  const ENTITY_PLURAL = "Plant Machines";
  ...
  const handleSearch = async () => {
    let { data, status, message } = await trackPromise(
      DefaultService.Entity_List("Settings_Fleet_Plant", { organisation_id: organisation.id })
    );
  ...
  const handleDelete = async (item) => {
    ...
    let { status, message } = await trackPromise(DefaultService.Entity_Delete("Settings_Fleet_Plant"));
    notify(message, status);
    await handleSearch();
  };
  ...
  const onFinish = async (values) => {
    ...
    */
    ...
    const { status, message } = record.id
      ? await trackPromise(DefaultService.Entity_Update("Settings_Fleet_Plant", record))
      : await trackPromise(DefaultService.Entity_Add("Settings_Fleet_Plant", record));
...
export default FleetManagement_Plants;
