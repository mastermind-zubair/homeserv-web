...
import Form_DiscountTag from "./Components/Form_DiscountTag";
import { useTranslation } from "react-i18next";
import DefaultService from "Services/API/DefaultService";
import Context from "Store/Context";
import PageTitle from "../_Common/PageTitle";

const DiscountTags = (props) => {
  const { t } = useTranslation();
  const { curOrg: organisation } = useContext(Context);
  const ENTITY = "Discount Tag";
  const ENTITY_PLURAL = "Discount Tags";
  const ENTITY_API_KEY = "QS_Discount_Tag";
          ...
          <h3 className="push-right text-right">
            <Tooltip title={`Do you have more ${ENTITY_PLURAL}? Click here to add another`}>
              <Button className="bg-success" icon={<PlusCircleOutlined />} onClick={handleEdit}>
                {t("quick_setup_add_new_discount_tags")}
              </Button>
            </Tooltip>
          </h3>
        </div>
