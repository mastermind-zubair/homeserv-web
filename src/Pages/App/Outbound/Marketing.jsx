...
import Form_MarketingCampaign from "./Components/Form_MarketingCampaign";
import Context from "Store/Context";
import PageTitle from "../_Common/PageTitle";
import LookupService from "Services/API/LookupService";
import environment from "Environment";
import _ from "lodash";
import Form_RunCampaign from "./Components/Form_RunCampaign";
import { SendEmail } from "Lib/SendGridHelper";
import Swal from "sweetalert2";
import { useTranslation } from "react-i18next";
const Marketing = (props) => {
  const ENTITY = "Marketing Campaign";
  const ENTITY_PLURAL = "Marketing Campaigns";
  const ENTITY_API_KEY = "Outbound_Marketing_Campaign";
  ...
  const runCampaign = async (customers, mCampaignRecord) => {
    ...
    let { status, message, data } = await trackPromise(
      DefaultService.POST("/market_campaign/run", {
        customers: customers,
        contents: mCampaignRecord.content,
        templateId: mCampaignRecord.template_id,
        organisation: organisation.name,
        subject: mCampaignRecord.subject,
        organisation_id: organisation.id,
      })
    );
    if (status) {
      alertify(
        `You just executed "${recordToEdit.name}" campaign successfully. Email sent to ${customers.length} customers`,
        status,
        10000
      );
      setShowRunCampaignForm(false);
      ...
      alertify(
        `System fails to execute "${recordToEdit.name}" campaign. Email cannot be sent at this time. ${message}`,
        status,
        10000
      );
    }
