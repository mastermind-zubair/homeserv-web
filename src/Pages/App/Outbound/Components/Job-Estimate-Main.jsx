...
const Job_Estimate_Main = ({ jid, qid, quoteEstimated }) => {
  //const { dtJob, dtQuote } = { dtJob: null, dtQuote: null };
  ...
  useEffect(async () => {
    ...
    if (!jid) {
      ...
      alertify("Not a valid job selected for quote", false);
    }
    ...
    const { data: org } = await trackPromise(
      DefaultService.Entity_Get("QS_Organisation", jb.organisation_id)
    );
    setOrganisation(org);

    //set job status to estimating
    const ftId = null; // AuthService.getCurrentTechnician().id;
    /* NOT 'ESTIMATING' is required */
  ...
  const makeRecommended = (jobOption) => {
    const options = jobOptions.map((o) => {
      return {
        ...o,
        is_recommended: o.title === jobOption.title ? true : false,
      };
    });
    //console.log(options);
    setJobOptions(options);
  ...
  const updateOptionTitle = (jobOption) => {
    ...
    options = options.map((o) => {
      return o.title === curJobOption.title
        ? { ...o, title: jobOption.title }
        : { ...o };
    });
    //console.log(options);
    setJobOptions(options);
  ...
  const presentEstimates = async () => {
    let quote = {
      ...
      job_id: job.id,
      technician_id: null, // AuthService.getCurrentTechnician().id,
    ...
    const { data, status, message } = qid
      ? await trackPromise(
          DefaultService.Entity_Update("Outbound_Quote", quote)
        )
  ...
  return (
    <>
      <Row>
        <Col span={12}>
          <Title level={4}>Add Estimates</Title>
        </Col>
  ...
  return (
    <>
      <Card
        size="small"
        className={`mb-2 ${
          jobOption.is_recommended && "box-orange box-double bg-orange"
        }`}
        ...
        headStyle={{
          ...
          borderLeft: `solid 3px ${
            jobOption.is_recommended ? "orange" : "#cadeff"
          }`,
      ...
      >
        <Row>
          <Col span={24}>
            <h2 className="danger">
              Total Price:{" "}
              <span className="text-danger text-large">
                ${getOptionTotal(jobOption)}
              </span>
            </h2>
          </Col>
        </Row>
        <Row>
          <Col span={6}>
            {(jobOption.items && jobOption.items.length) || 0} Item(s)
          </Col>
          <Col span={18} className="text-right push-right">
            <Space>
              <Button
                type="dashed"
                danger
                onClick={() => handleAddNewItem(jobOption)}
              >
                Add Items
              </Button>
        ...
        <Row className="mt-2">
          <Col span={18} className="mt-2">
            ...
            />
            <span
              className={`ml-1 mt-2 text-small ${
                jobOption.is_recommended ? "text-success text-bold" : ""
              }`}
            >
              Recommended
            </span>
          ...
          <Col span={6}>
            <Popconfirm
              ...
              onConfirm={() => handleRemoveOption(jobOption)}
            >
  ...
  return (
    <>
      <Modal
        ...
        onCancel={handleCancel}
        footer={[
          <FormButtons {...{ form, handleCancel, ENTITY: "Option Name" }} />,
        ]}
      >
        <Form
          form={form}
            ...
            <Button
              ...
              onClick={() =>
                license.id === 2
                  ...
                  : notify(
                      "This feature is only available for GROW license of this application.",
                      false
                    )
              }
              disabled={false}
      ...
      >
        ...
        >
          <div className="">
            ...
            >
              <Input
                placeholder="Please enter a new item name"
                maxLength={100}
              />
            </Form.Item>
            ...
            <Form.Item
              ...
              label="Description"
              rules={[
                { required: true, message: "A brief description is required" },
              ]}
              hasFeedback
              className="one-row-item"
            >
            ...
            <Row gutter={10}>
              <Col span={12}>
                <Form.Item
                  name="tax_rate"
                  label="Tax Rate"
                  initialValue={organisation.tax_rate}
                >
...
const AddItemsPB = ({
  ...
  const getCategoriesByIndustry = async (i) => {
    let { data: cTree } = await trackPromise(
      DefaultService.Entity_List("PriceBook_ServiceCategory", {
        organisation_id: job.organisation_id,
        industry_id: i,
      })
    );
  ...
  const selectService = (service) => {
    ...
    let file_path =
      service.media && service.media.length > 0 && service.media[0].file_path;

    setCurServiceMediaEndpoint(
      `${environment.PATH_SERVICE_MEDIA}/${file_path}`
    );
    setCurServiceQty(1);
  ...
  const columns = [
...

    {
      ...
      cellRender: (d) => {
        let file_path =
          d.data.media && d.data.media.length > 0 && d.data.media[0].file_path;
        return (
          <>
            <Button
              style={{ height: "80px" }}
              type="link"
              onClick={() => selectService(d.data)}
            >
              <img
                src={`${environment.PATH_SERVICE_MEDIA}/${file_path}`}
                style={{ width: "80px" }}
              />
            </Button>
          </>
  ...
  return (
    <>
      <Modal
        ...
        footer={[]}
      >
        <h4>Select a service category:</h4>
        <TreeSelect
          style={{ width: "100%" }}
